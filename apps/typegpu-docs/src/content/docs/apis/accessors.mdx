...
The [Slots and Accessors](/TypeGPU/examples#example=simple--slots-and-accessors) example combines both: a slot picks the pattern function, and an accessor is bound to a constant, a uniform buffer, and a GPU function in three different pipelines.
